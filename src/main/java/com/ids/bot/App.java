...
public class App {
    ...
    /**
     * Public static factory shared by App, ParallelRunner, and tests.
     *
     * --disable-features=AsyncDns: without this Chrome uses its own resolver and
     * cannot resolve techmarket.lab, bypassing the split-DNS setup.
     */
    ...
    /**
     * Overrides UA and Client Hints via untyped CDP command.
     *
     * DESIGN DECISION: Selenium's typed CDP bindings (org.openqa.selenium.devtools.v125...)
     * only support up to Chrome v125. On a newer Chrome the "Unable to find CDP
     * implementation matching" warning appears and the typed API becomes fragile.
     * Using untyped executeCdpCommand instead keeps both the UA string and Client Hints
     * headers (sec-ch-ua etc.) consistent.
     */
    ...
    /**
     * Disables the browser cache via CDP.
     *
     * ORDER MATTERS: setCacheDisabled silently no-ops unless Network.enable is called first.
     * (This was discovered in project history when silent cache failures caused revisited
     * pages to disappear from Zeek/Nginx logs entirely.)
     */
